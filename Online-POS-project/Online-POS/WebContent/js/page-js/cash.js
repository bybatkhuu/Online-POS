...
function startTime() {
	...
	var dateTimeStr = year + "." + month + "." + date + " - " + hour + ":"
			+ minute + ":" + second + " "/* + " <b class='blue'>" */+ day /* + "</b>" */;
...
function errorDialog(message) {
	$("#errorMessage").html(
			"<strong><i class='icon-remove'></i></strong>&nbsp; " + message);
			...
			$("#barcode").autocomplete(
					{
						source : function(request, response) {
							var matches = $.map(barcodes, function(tag) {
								if (tag.toUpperCase().indexOf(
										request.term.toUpperCase()) === 0) {
									return tag;
								}
							});
							response(matches);
		...
		success : function(result) {
			...
			if (jsonData.hasPrice == "true") {
				...
				$slider.slider("option", "values", [
						parseFloat(jsonData.minPrice),
						parseFloat(jsonData.maxPrice) ]);
			}
		}
	});
			...
			if (!isNaN(quantity)) {
				if (quantity > 0) {
					...
					if (isBarcode == 1) {
						...
						alert("'"
								+ barcode
								+ "' гэсэн баркодтай бараа энэ тасагт олдохгүй байна!");
					}
				} else {
...
function bindForm(element) {
	...
	var oldDisCountPercent = parseFloat($(element).children().eq(
			thDisCountPercent).text().trim());
				...
				function(event) {
					...
					if (event.which == 37 || event.which == 39) {
						...
						if ($("#newQuant").is(":focus")) {
							...
							bindForm(element);
						} else if ($("#newQuant1").is(":focus")) {
							$("#newQuant1").remove();
							leftKey = 1;
							$(element).children().eq(thDisCountPercent).text(
									oldDisCountPercent);
							bindForm(element);
						}
		...
		$("#newQuant1")
				.keydown(
						function(event) {
							...
							if (event.which == 13) {
								updateFromTableDisCount(element, id,
										oldDisCountPercent);
							}
							if (event.which == 37 || event.which == 39) {
								event.preventDefault();
								if ($("#newQuant").is(":focus")) {
									leftKey = 2;
									$("#newQuant").remove();
									$(element).children().eq(thQuant).text(
											oldQuant);
									bindForm(element);
								} else if ($("#newQuant1").is(":focus")) {
									$("#newQuant1").remove();
									leftKey = 1;
									$(element).children().eq(thDisCountPercent)
											.text(oldDisCountPercent);
									bindForm(element);
								}
...
function editPrice() {
	if ($("#unitPrice").val() != "") {
		if (isNumber($("#unitPrice").val())) {
			var priceCheck = $("#tableBody > tr[class='success']").children()
					.eq(thPrice).text();
			if (priceCheck > $("#unitPrice").val()) {
				...
				alert($("#tableBody > tr[class='success']").children().eq(
						thPrice).text().trim()
						+ "-с бага тоо оруулна уу");
				$("#unitPrice").val(
						$("#tableBody > tr[class='success']").children().eq(
								thPrice).text().trim());
			}
		} else {
			alert("Үнэ засах хэсэгт зөвхөн тоо оруулна уу!");
			$("#unitPrice").val(
					$("#tableBody > tr[class='success']").children()
							.eq(thPrice).text().trim());
		}
	} else {
		alert("Засах үнээ оруулна уу!");
		$("#unitPrice").val(
				$("#tableBody > tr[class='success']").children().eq(thPrice)
						.text());
	}
...
function loadPrintView() {
	...
	for ( var i = 0; i < $("#tableBody > tr").size(); i++) {
		...
		str = str
				+ "<div class='col-xs-6' style = 'padding-left:5px;margin-left:5px'>"
				+ $("#tableBody > tr:eq(" + i + ")").children().eq(thName)
						.html() + "</div>";
		str = str
				+ "<div class='col-xs-1' style = 'padding-left:0px;margin-left:0px' >"
				+ $("#tableBody > tr:eq(" + i + ")").children().eq(thQuant)
						.html() + "</div>";
		str = str
				+ "<div class='col-xs-2' style = 'padding-left:0px;margin-left:0px'>"
				+ $("#tableBody > tr:eq(" + i + ")").children().eq(thPrice)
						.html() + "</div>";
		str = str
				+ "<div class='col-xs-2' style = 'padding-left:0px;margin-left:0px'>"
				+ $("#tableBody > tr:eq(" + i + ")").children().eq(thTotal)
						.html() + "</div>";
		str = str + "</div>";
...
function initTableEvents() {
	...
	$("#unitPrice").val(
			$("#tableBody > tr[class='success']").children().eq(thPrice).text()
					.trim());
	$("#itemName").val(
			$("#tableBody > tr[class='success']").children().eq(thName).text()
					.trim());
	$("#unit").text(
			$("#tableBody > tr[class='success']").children().eq(thUnit).text()
					.trim());
	$("#quantity").val("1");
	$("#barcode").val("");
}
			...
			.keydown(
					function(event) {
						...
						if (event.keyCode == 122) {
							event.preventDefault();
							// silverPen start
							if (!$("#searchItemsDialog").parents(".ui-dialog")
									.is(":visible")) {
								$("#customerCheck").trigger('click');
								if ($("#customerCheck").prop("checked")) {
									$("#customers").focus();
							...
							if (!$("#searchItemsDialog").parents(".ui-dialog")
								...
								if ($("#discountPercent").val().trim() == "") {
									if ($("#tableBody > tr").size() != 0) {
										...
										localStorage.setItem("talon", $(
												"#talon").val().trim());
										localStorage.setItem("customerCheck",
												$("#customerCheck").prop(
														"checked").toString());
										localStorage.setItem("customers", $(
												"#customers").val().trim());
										localStorage.setItem("bankCheck", $(
												"#bankCheck").prop("checked")
												.toString());
									...
									/*
									 * }else{ alert("Төлбөр Хийгээгүй байна"); }
									 */
								} else {
...
//								}
						...
						// f7
							...
							if (!$("#searchItemsDialog").parents(".ui-dialog")
									.is(":visible")) {
								if(document.getElementById("paid").disabled == false){
									$("#paid").val("");
									$("#paid").focus();
								}else{
									alert('Бараа орж ирээгүй байна');
								}
							}
						}
						// f8
						...
						// f9
							...
							if (!$("#searchItemsDialog").parents(".ui-dialog")
								...
								/*
								 * if ($("#tableBody >
								 * tr[class='success']").size() == 1) {
								 * $("#unitPrice").prop("disabled", false);
								 * $("#unitPrice").focus(); } else {
								 * $("#unitPrice").prop("disabled", true); }
								 */
								// costca end
							} else {
						...
						// Up arrow
							...
							if (!$("#barcode").is(":focus")) {
								...
								if (!$("#searchItemsDialog").parents(
									...
									var i = $(
											"#searchResultBody > tr[class='success']")
											.index();
									...
									if (s > 0) {
										if (i <= s - 1) {
											if (i > 0) {
												...
												i = $("#searchResultBody > tr")
														.size() - 1;
											}
											$(
													"#searchResultBody > tr[class='success']")
													.removeClass();
											$("#searchResultBody > tr").eq(i)
													.addClass("success");
										}
										$("#searchItemsDialog").scrollTop(
												i * 12);
									}
								}
							}
						...
						// costca start
						/*
						 * else if (event.which == 46) { if
						 * (!$("#barcode").is(":focus") &&
						 * !$("#discountPercent").is(":focus") &&
						 * !$("#searchByName").is(":focus")&
						 * !$("#searchByBarcode").is(":focus") &&
						 * !$("#paid").is(":focus")&&
						 * !$("#quantity").is(":focus") &&
						 * !$("#cardNumber").is(":focus")&&
						 * !$("#talon").is(":focus") &&
						 * !$("#newQuant").is(":focus")&&
						 * !$("#newQuant1").is(":focus") ){
						 * $("#deleteButton").trigger("click"); checkAll(); } }
						 */
						// costca end
					});
	...
	$("#barcode").keypress(
			function(event) {
				if (event.which == 13) {
					event.preventDefault();
					addItem($("#barcode").val().trim(), $("#quantity").val()
							.trim(), $("#assetAccounts").val().trim());
				}
			});
	$("#quantity")
			.keypress(
					function(event) {
						if (event.which == 13) {
							event.preventDefault();
							if ($("#barcode").val() != "") {
								addItem($("#barcode").val().trim(), $(
										"#quantity").val().trim(), $(
										"#assetAccounts").val().trim());
							} else {
								if ($(this).val() != "") {
									if (!isNaN($(this).val())) {
										if ($(this).val() > 0) {
	...
	$("#addItem").click(
			function(event) {
				event.prventDeafult();
				addItem($("#barcode").val().trim(),
						$("#quantity").val().trim(), $("#assetAccounts").val()
								.trim());
			});
	...
	/*
	 * $("body").click(function(event) { if (event.target.nodeName != "TD") { if
	 * (event.target.getAttribute('id') != "updateButton" &&
	 * event.target.getAttribute('id') != "deleteButton") { $("#tableBody >
	 * tr[class='success']").removeClass(); checkAll(); } } });
	 */
	...
	$("#paid").keyup(function(event) {
		if ($("#payOff").val() > 0) {
			if (event.which == 13 && $("#return").val() >= 0) {
				event.preventDefault();
				alert("Хариулт мөнгө :" + $("#return").val());
				loadPrintView();
				purchase();
			}
		}
			...
			function(event) {
				...
				if ($("#selectItems").is(':focus')) {
					if ($("#searchResultBody > tr").size() > 0) {
						var i = $("#searchResultBody > tr[class='success']")
								.index();
						addItem($("#searchResultBody > tr").eq(i).children()
								.eq(1).text().trim(), 1, $(
								"#searchResultBody > tr").eq(i).children()
								.eq(3).text().trim());
	...
	$("#searchItems")
			.click(
					function(event) {
						event.preventDefault();
						if ($("#searchByName").val().trim() != ""
								|| $("#searchByBarcode").val().trim() != "") {
									...
									.ajax({
										...
										data : {
											...
											"barcode" : $("#searchByBarcode")
													.val().trim(),
											"minPrice" : $("#searchByMinPrice")
													.val().trim(),
											"maxPrice" : $("#searchByMaxPrice")
													.val().trim(),
											"assetAcc" : $("#assetAccounts")
													.val().trim()
										},
										success : function(result) {
...
//	loadPrintView();
	...
	if ($("#tableBody > tr").size() <= 0) {
		...
		for ( var i = 0; rowSize > i; i++) {
			total = total
					+ parseFloat($("#tableBody > tr:eq(" + i + ")").children()
							.eq(thTotal).text().trim());
		}
		$("#calTotal").val(total);
